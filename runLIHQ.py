...
def run(face, audio_super = '/content/LIHQ/input/audio', ref_vid = '/content/LIHQ/input/ref_vid/syn_reference.mp4', ref_vid_offset = [0], frame_int = None, clear_outputs=True, save_path = None):
    ...
    if audio_super[-1:] != '/':
        ...
        print("audion folder not checked yet")
    ...
    generator, kp_detector = load_checkpoints(config_path='./first_order_model/config/vox-256.yaml', checkpoint_path='./first_order_model/vox-cpk.pth.tar', cpu=True)
    i = 0
    # generator.to(mps_device)
    # kp_detector.to(mps_device)
...
face_path = './input/face/examples/2372448.png'

# run(face = face_path)

run(face = face_path, audio_super = './input/audio', ref_vid = './input/ref_vid/syn_reference.mp4', ref_vid_offset = [0], frame_int = None, clear_outputs=True, save_path = None)